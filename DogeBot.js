...
gaya1 = setting.gaya1
gaya2 = setting.gaya2
const fakeimage = fs.readFileSync ('./media/Phantompic1.jpg')
const thumb = fs.readFileSync ('./media/Phantomtb.jpg')

//══════════[ Developer Number ]══════════//

developerNo = '919744037374'
developerName= '🦄Dream Guy Razi'
...
module.exports = PhantomRaziOP = async (PhantomRaziOP, mek, _welkom) => {
	try {
		...
		const botNumber = PhantomRaziOP.user.jid
		const ownerNumber = [`${owner}@s.whatsapp.net`, `919744037374@s.whatsapp.net`]
		const isGroup = from.endsWith('@g.us')
		let sender = isGroup ? mek.participant : mek.key.remoteJid
		let senderr = mek.key.fromMe ? PhantomRaziOP.user.jid : mek.key.remoteJid.endsWith('@g.us') ? mek.participant : mek.key.remoteJid
		const groupMetadata = isGroup ? await PhantomRaziOP.groupMetadata(from) : ''.toString
        ...
        m = simple.smsg(PhantomRaziOP, mek)
        var pes = (type === 'conversation' && mek.message.conversation) ? mek.message.conversation : (type == 'imageMessage') && mek.message.imageMessage.caption ? mek.message.imageMessage.caption : (type == 'videoMessage') && mek.message.videoMessage.caption ? mek.message.videoMessage.caption : (type == 'extendedTextMessage') && mek.message.extendedTextMessage.text ? mek.message.extendedTextMessage.text : ''
        const messagesD = pes.slice(0).trim().split(/ +/).shift().toLowerCase()
		const conts = mek.key.fromMe ? PhantomRaziOP.user.jid : PhantomRaziOP.contacts[sender] || { notify: jid.replace(/@.+/, '') }
        const pushname = mek.key.fromMe ? PhantomRaziOP.user.name : conts.notify || conts.vname || conts.name || '-'    
		...
		let bio_nya = await PhantomRaziOP.getStatus(sender)
		try {
			...
			try {
		pporang = await PhantomRaziOP.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
		      } catch {
...
PhantomRaziOP.sendMessage(from, hasil, type, options).catch(e => {
fetch(link).then((hasil) => {
PhantomRaziOP.sendMessage(from, hasil, type, options).catch(e => {
PhantomRaziOP.sendMessage(from, { url : link }, type, options).catch(e => {
reply
console.log(e)
})
})
      ...
      download(url, "./stik" + names + ".png", async function () {
        ...
        exec(
          ...
          (err) => {
            ...
            PhantomRaziOP.sendMessage(to, media, MessageType.sticker, { quoted: mek });
        ...
        const sendWebp = async(from, url) => {
                ...
                download(url, './temp' + names + '.png', async function () {
                    ...
                    exec(`ffmpeg -i ${ajg} -vcodec libwebp -filter:v fps=fps=20 -lossless 1 -loop 0 -preset default -an -vsync 0 -s 512:512 ${palak}`, (err) => {
                        ...
                        PhantomRaziOP.sendMessage(from, media, MessageType.sticker, {quoted:mek})
                        fs.unlinkSync(ajg)
                        fs.unlinkSync(palak)
                    });
                });
...
PhantomRaziOP.sendMessage(from, buttonMessages, buttonsMessage, {
quoted: mek
})
}
...
PhantomRaziOP.sendMessage(id, buttonMessage, MessageType.buttonsMessage, options)
}
const sendButImage = async(id, text1, desc1, gam1, but = [], options = {}) => {
kma = gam1
mhan = await PhantomRaziOP.prepareMessage(from, kma, image)
...
PhantomRaziOP.sendMessage(id, buttonMessages, MessageType.buttonsMessage, options)
}
const sendButVideo = async(id, text1, desc1, vid1, but = [], options = {}) => {
kma = vid1
mhan = await PhantomRaziOP.prepareMessage(from, kma, video)
const buttonMessages = {
...
kma = gam1
mhan = await PhantomRaziOP.prepareMessage(from, kma, location)
const buttonMessages = {
...
let po = PhantomRaziOP.prepareMessageFromContent(from, {"listMessage": {"title": title,"description": desc,"buttonText": "𝗠𝗘𝗡𝗨","footerText": `${tanggal}`,"listType": "SINGLE_SELECT","sections": list}}, {})
return PhantomRaziOP.relayWAMessage(po, {waitForAck: true})
}
const reply = (teks) => {
PhantomRaziOP.sendMessage(from, teks, text, {quoted:mek})
}
const sendMess = (hehe, teks) => {
PhantomRaziOP.sendMessage(hehe, teks, text)
}
const isUrl = (url) => {
return url.match(new RegExp(/https?:\/\/(www\.)?[-a-zA-Z0-9@:%.+~#=]{1,256}\.[a-zA-Z0-9()]{1,6}\b([-a-zA-Z0-9()@:%+.~#?&/=]*)/, 'gi'))
        }
const mentions = (teks, memberr, id) => {
(id == null || id == undefined || id == false) ? PhantomRaziOP.sendMessage(from, teks.trim(), extendedText, { contextInfo: { "mentionedJid": memberr } }) : PhantomRaziOP.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": memberr } })
}
const costum = (pesan, tipe, target, target2) => {
PhantomRaziOP.sendMessage(from, pesan, tipe, { quoted: { key: { fromMe: false, participant: `${target}`, ...(from ? { remoteJid: from } : {}) }, message: { conversation: `${target2}` } } })
}
...
const textImg = (teks) => { return PhantomRaziOP.sendMessage(from, teks, text, {quoted: fgi, thumbnail: fs.readFileSync('./media/Phantomtb.jpg')})}
const fakeitem = (teks) => { PhantomRaziOP.sendMessage(from, teks, text, { quoted: { key:{ fromMe:false, participant:`0@s.whatsapp.net`, ...(from ? { remoteJid :"6289523258649-1604595598@g.us" }: {})},message:{"orderMessage":{"orderId":"174238614569481","thumbnail":fs.readFileSync("./media/Phantomtb.jpg"),"itemCount":9999999999,"status":"INQUIRY","surface":"CATALOG","message": `${ucapanWaktu} ${pushname}`,"token":"AR6xBKbXZn0Xwmu76Ksyd7rnxI+Rx87HfinVlW4lwXa6JA=="}}},contextInfo: {"forwardingScore":999,"isForwarded":true}, sendEphemeral: true})}
...
PhantomRaziOP.sendMessage(to, media, type, { quoted: fgi, mimetype: mime, caption: text,contextInfo: {"mentionedJid": mids}})
...
let anugroupsend = await PhantomRaziOP.groupMetadata(from)
let members = anugroupsend.participants
let ane = []
for (let i of members){
ane.push(i.jid)
}
PhantomRaziOP.sendMessage(from, {text:text, jpegThumbnail:fs.readFileSync('media/Phantompic1.jpg')}, 'extendedTextMessage', {contextInfo: {"mentionedJid": ane}})
}
const hideTagKontak = async function(from, nomor, nama){
let vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + 'ORG:Kontak\n' + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
let anuvcardoke = await PhantomRaziOP.groupMetadata(from)
let members = anuvcardoke.participants
let ane = []
for (let i of members){
ane.push(i.jid)
}
PhantomRaziOP.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {contextInfo: {"mentionedJid": ane}})
}
const sendKontak = (from, nomor, nama) => {
const vcard = 'BEGIN:VCARD\n' + 'VERSION:3.0\n' + 'FN:' + nama + '\n' + `ORG:Developer ${botname}\n` + 'TEL;type=CELL;type=VOICE;waid=' + nomor + ':+' + nomor + '\n' + 'END:VCARD'
PhantomRaziOP.sendMessage(from, { displayname: nama, vcard: vcard}, MessageType.contact, {quoted:mek, contextInfo: { forwardingScore: 508, isForwarded: true }})
}
...
for (let anji of setik){
				if (budy === anji){
					...
					PhantomRaziOP.sendMessage(from, result, sticker, { quoted: mek})
					}
			}
			for (let anju of vien){
				if (budy === anju){
					result = fs.readFileSync(`./media/vn/${anju}.mp3`)
					PhantomRaziOP.sendMessage(from, result, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
					}
			}
			for (let anjh of imagi){
				if (budy === anjh){
					result = fs.readFileSync(`./media/image/${anjh}.jpg`)
					PhantomRaziOP.sendMessage(from, result, image, { quoted: mek})
					}
				  }
...
╰───「 *🥳ʟᴇᴠᴇʟ ᴜᴘ🥳* 」`)
          ...
          sendButton(from, lvlup, '*Phantom Bot*', but)
...
setTimeout(() => {
PhantomRaziOP.groupRemove(from, [kic]).catch((e) => { reply(`BOTS MUST BE ADMIN`) })
}, 0)
}
...
PhantomRaziOP.groupRemove(from, [sender])
}     

//══════════[ Dll ]══════════//

if (autoread){
PhantomRaziOP.chatRead(from, "read")
} else if (autoketik) {
PhantomRaziOP.updatePresence(from, Presence.composing)
} else if (autovn) {
PhantomRaziOP.updatePresence(from, Presence.recording)
}
...
const { wa_version, os_version } = PhantomRaziOP.user.phone
...
${gaya1} *Wa Version :* ${PhantomRaziOP.user.phone.wa_version}
...
PhantomRaziOP.sendMessage(from, listMsg, MessageType.listMessage, {contextInfo: { mentionedJid: [senderr]},quoted:fgi})
break
case 'allmenu':
...
PhantomRaziOP.sendMessage(from, { contentText: `${menu}`, footerText: `*_${tanggal}_*`, buttons: [{ buttonId: `${prefix}command`, buttonText: { displayText: 'BACK ⬅️' }, type: 1 },{ buttonId: `${prefix}abcde`, buttonText: { displayText: '\n\nAKU ADALAH GAY' }, type: 1 } ], headerType: 'LOCATION', locationMessage: { degreesLatitude: '', degreesLongitude: '', jpegThumbnail: fakeimage, contextInfo: {mentionedJid: [sender]}}}, 'buttonsMessage')
break
case 'asupanmenu':
...
gambar = fs.readFileSync('./media/Phantomd.jpg')
menunya = `*「 PAYMENT 」*
...
Please Choose Whether Document, Audio or Video Below`
        sendButMessage(from, result, `*Phantom Bot*`, [
          {
...
case 'spotify':{
    ...
    await PhantomRaziOP.sendMessage(from, thumbnail, image, { quoted: mek, caption: ini_txt })
    get_audio = await getBuffer(get_result.link)
    await PhantomRaziOP.sendMessage(from, get_audio, audio, { mimetype: 'audio/mpeg', filename: `${get_result.title}.mp3`, quoted: mek})
    }
    break
   case 'soundcloud':
				...
				zee.SoundCloud(`${q}`).then(async (data) => {
                    ...
                    PhantomRaziOP.sendMessage(from , await getBuffer(data.medias[0].url), audio,{ quoted: mek, mimetype: 'audio/mp4' })
				})
			break
	case 'telesticker': case 'telegramsticker': case 'tstiker': {
			...
			PhantomRaziOP.sendMessage(from, await getBuffer(telestc[i].url), sticker, {mimetype:'image/webp',quoted: mek})
			}
		}
		break
case 'tiktoknowm':
   case 'tiktok':
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					PhantomRaziOP.sendMessage(from,noweem , MessageType.video, {mimetype: 'video/mp4',quoted: mek})
					})
...
case 'tiktokwm':
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					PhantomRaziOP.sendMessage(from,weem , MessageType.video, {mimetype: 'video/mp4',quoted: mek})
					})
				}).catch((err) => reply(`Invalid link`))
 ...
 case 'tiktokmp3':
			...
			.then(result => {
				...
				.then(async (a) => {
					...
					PhantomRaziOP.sendMessage(from,audnha , MessageType.document, {mimetype: 'audio/mp4',filename: `Tiktok Music.mp3`,quoted: mek})
					})
				}).catch((err) => reply(`Invalid link`))
...
PhantomRaziOP.sendMessage(from, mp4, video, { quoted: mek, caption: `Here is your video🐶` })
break
case 'mp3':
reply(mess.wait)
bo = args.join(" ")
...
PhantomRaziOP.sendMessage(from, mp3, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
break
case 'ytmp3':
...
PhantomRaziOP.sendMessage(from, ytmp3, audio, {mimetype:"audio/mp4", quoted:mek})
break
case 'ytmp4':
...
PhantomRaziOP.sendMessage(from, ytmp4, video, {caption:`Done✓`, thumbnail:Buffer.alloc(0), quoted:mek})
break
...
--> Razi*/
                ...
                reply(mess.wait)
                    getBuffer(`https://api.lolhuman.xyz/api/random/${command}?apikey=${Lolhumanbykur}`).then((gambar) => {
                        PhantomRaziOP.sendMessage(from, gambar, image, { quoted: mek })
                    })
                    break
                   case 'fox':  
                   anufox= await fetchJson(`https://some-random-api.ml/img/fox`)
                   anu1 = await getBuffer(anufox.link)
                   PhantomRaziOP.sendMessage(from, anu1, image, {caption: `Here you go!`, quoted: mek})
                   break
                   case 'dog':  
...
break
             case 'sao':
				...
				PhantomRaziOP.updatePresence(from, Presence.composing) 
                 ...
                 PhantomRaziOP.sendMessage(from, swordartonline, image, {quoted: mek, caption: 'swort art online\nPhantomBot'})
				 break
				case 'hsdxd':
				reply(mess.wait)
				 PhantomRaziOP.updatePresence(from, Presence.composing) 
                 ...
                 PhantomRaziOP.sendMessage(from, highschooldxd, image, {quoted: mek, caption: 'Here u go!'})
				break
				 case 'lovelive':
				reply(mess.wait)
				 PhantomRaziOP.updatePresence(from, Presence.composing) 
                 ...
                 PhantomRaziOP.sendMessage(from, lovelive, image, {quoted: mek, caption: 'Here u go!'})
				break
...
sendButVideo(from, pll, `*Phantom Bot*`, get_result, [      
{
buttonId: `${prefix+command}`,
buttonText: {
displayText: `NEXT ➡️`,
...
//----> 1 TEXT <----//
                ...
                case 'freefire':          
                    if (args.length == 0) return reply(`Example: ${prefix + command} Razi`)
                    ini_txt = args.join(" ")
                    getBuffer(`https://api.lolhuman.xyz/api/ephoto1/${command}?apikey=${lolkey}&text=${ini_txt}`).then((gambar) => {
                        PhantomRaziOP.sendMessage(from, gambar, image, {thumbnail: Buffer.alloc(0), caption: `Here you go!`, quoted : mek}) 
                    })
                    break
case 'blackpink':
case 'halloween':
...
PhantomRaziOP.sendMessage(from, oke, image, {quoted: mek, caption: 'Here u go!😛'})
break
case 'hartatahta': 
...
PhantomRaziOP.sendMessage(from, harta, image, {quoted: mek, caption: 'Here u go!😛'})
break
...
PhantomRaziOP.sendMessage(from, pornhub, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'pornhub':
...
PhantomRaziOP.sendMessage(from, girlneko, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'sadboy':
...
PhantomRaziOP.sendMessage(from, girlneko2, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'makerkaneki':
...
PhantomRaziOP.sendMessage(from, girlneko8383, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'rem':
...
PhantomRaziOP.sendMessage(from, girlneko11111, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'lolimaker':
...
PhantomRaziOP.sendMessage(from, girlnekojkjk, image, {caption: `Here u go!😛`, quoted: mek})
break
case 'gura':
...
PhantomRaziOP.sendMessage(from, girlnekoooo, image, {caption: `Here u go!😛`, quoted: mek})
break
                         case 'wolf3':
                   ...
                   PhantomRaziOP.sendMessage(from, anuwolf3, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it?`, quoted: mek})
                   break
                          case 't3d':
                   ...
                   PhantomRaziOP.sendMessage(from, anut3d, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'logoa':
       
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} Razi&lol*`)
                   var F = body.slice(7)
				   var F1 = F.split("|")[0];
				   var F2 = F.split("|")[1]; 
                   reply(mess.wait)
                   anulogoa = await getBuffer(`${ApiZeks}/api/logoaveng?text1=${F1}&text2=${F2}&apikey=${zeksApikey}`)
                   PhantomRaziOP.sendMessage(from, anulogoa, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'phlogo':  
                   ...
                   PhantomRaziOP.sendMessage(from, anuphlogo, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'marvel3':
                   ...
                   PhantomRaziOP.sendMessage(from, anumrvl3, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'leavest':  
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} Razi*`)
                   F = body.slice(9)
                   ...
                   PhantomRaziOP.sendMessage(from, anulvst, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break                   
       case 'notewrite':
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} Razi*`)
                   F = body.slice(7)
                   ...
                   PhantomRaziOP.sendMessage(from, anunw, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break
       case 'neon2':
                   if (args.length < 1) return reply(`[  ×  ] Example :\n*${prefix}${command} Razi*`)
                   F = body.slice(7)
                   ...
                   PhantomRaziOP.sendMessage(from, anunion2, image, {thumbnail: Buffer.alloc(0),caption: `OK it's done\n\nHow is it? `, quoted: mek})
                   break 
...
+ 'VERSION:3.0\n'
+ `FN:${ownername}\n`
+ `ORG: Owner Of Phantom Bot ;\n`
+ `TEL;type=CELL;type=VOICE;waid=${owner}:${owner}\n`
+ 'END:VCARD'.trim()
PhantomRaziOP.sendMessage(from, {displayName: `The owner ${botname}`, vcard: vcard2}, contact, 
{ quoted: fgi, 
})
...
PhantomRaziOP.sendMessage(from, devsound, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek })
break
case 'sc':
case 'script':
case 'sourcode':
...
PhantomRaziOP.deleteMessage(from, { id: mek.message.extendedTextMessage.contextInfo.stanzaId, remoteJid: from, fromMe: true })
break
...
await PhantomRaziOP.sendMessage(from, ini_buffer, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek})
break
...
PhantomRaziOP.sendMessage(from, buff, image, {quoted: mek, caption : teks})
break
...
//══════════[ CONVERT FEATURES ]══════════//
      ...
      case 'squirrel':
        ...
        media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedia012)
        ran = getRandom('.mp3')
        exec(`ffmpeg -i ${media} -af atempo=1/2,asetrate=44500*2/1 ${ran}`, (err, stderr, stdout) => {
          fs.unlinkSync(media)
          if (err) return reply('Error!')
          hah = fs.readFileSync(ran)
          PhantomRaziOP.sendMessage(from, hah, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek })
          fs.unlinkSync(ran)
        })
        break
      case 'blub':
        ...
        media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediakekek)
        ran = getRandom('.mp3')
      ...
      case 'ghost':
        ...
        media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedia777)
        ran = getRandom('.mp3')
      ...
      case 'cutesound':
        ...
        media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedia100)
        ran = getRandom('.mp3')
        exec(`ffmpeg -i ${media} -filter:a "atempo=1.0,asetrate=50000" ${ran}`, (err, stderr, stdout) => {
          fs.unlinkSync(media)
          if (err) return reply('Error!')
          riu = fs.readFileSync(ran)
          PhantomRaziOP.sendMessage(from, riu, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek })
          fs.unlinkSync(ran)
        })
        break
case 'slowmo': case 'slow':{
								try {
										...
										media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedia22)
										ran = getRandom('.mp3')
										exec(`ffmpeg -i ${media} -filter:a "atempo=0.7,asetrate=44100" ${ran}`, (err, stderr, stdout) => {
											fs.unlinkSync(media)
											if (err) return reply('Error!')
											uhh = fs.readFileSync(ran)
											PhantomRaziOP.sendMessage(from, uhh, audio, {mimetype: 'audio/mp4', ptt:true, quoted: mek})
											fs.unlinkSync(ran)
											})
											} catch (e) {
												reply('Error!')
...
case 'fast':{
									try {
										...
										media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedia11)
										ran = getRandom('.mp3')
...
case 'vibra': case 'vibrato':{
									...
									media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedia33)
									ran = getRandom('.mp3')
    ...
    case 'nightcore':            
		  ...
		  core = await PhantomRaziOP.downloadAndSaveMediaMessage(night)
		  ran = getRandom('.mp3')
		  reply(mess.wait)
		  ...
		  if (err) return reply('Error!')
		  hah = fs.readFileSync(ran)
		  PhantomRaziOP.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:false, quoted: mek, ptt: true})
		  fs.unlinkSync(ran)
		  })
	      break
 case 'vnsec':
encmediam = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediam = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediam)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(mediam)
						PhantomRaziOP.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: cokmatane, ptt: true, quoted:mek})
						fs.unlinkSync(mediam)
				break
				case 'vidsec':
				encmedian = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					median = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedian)
					cokmatane = Number(args[0])
					hah = fs.readFileSync(median)
						PhantomRaziOP.sendMessage(from, hah, video, {mimetype: 'video/mp4', duration: cokmatane, quoted: mek})
						fs.unlinkSync(median)
				break
 case 'robot':
encmedial = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
medial = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedial)
ran = getRandom('.mp3')
exec(`ffmpeg -i ${medial} -filter_complex "afftfilt=real='hypot(re,im)*sin(0)':imag='hypot(re,im)*cos(0)':win_size=512:overlap=0.75" ${ran}`, (err, stderr, stdout) => {
fs.unlinkSync(medial)
if (err) return reply(mess.error.api)
hah = fs.readFileSync(ran)
PhantomRaziOP.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true, quoted: mek})
fs.unlinkSync(ran)
})
break
case 'fat':
					encmediaz = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
					mediaz = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediaz)
					ran = getRandom('.mp3')
					exec(`ffmpeg -i ${mediaz} -filter:a "atempo=1.6,asetrate=22100" ${ran}`, (err, stderr, stdout) => {
						fs.unlinkSync(mediaz)
						if (err) return ephe('Error!')
						hah = fs.readFileSync(ran)
					PhantomRaziOP.sendMessage(from, hah, audio, {mimetype: 'audio/mp4', ptt:true, duration: 359996400, quoted:mek})
						fs.unlinkSync(ran)
					})
					break
case 'reverse':
	encmediau = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	mediau = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediau)
	ran = getRandom('.mp3')
...
case 'bass':                 
					...
					mediao = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediao)
					ran = getRandom('.mp3')
 ...
 case 'tomp3':
					...
					mediad = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediad)
					ran = getRandom('.mp4')
					exec(`ffmpeg -i ${mediad} ${ran}`, (err) => {
						fs.unlinkSync(mediad)
						if (err) return reply(mess.error.api)
						mhee = fs.readFileSync(ran)
						PhantomRaziOP.sendMessage(from, mhee, audio, { mimetype: 'audio/mp4', ptt: true, quoted: mek })
						fs.unlinkSync(ran)
					})
					break
 case 'toimg':
...
media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediatoimg)
ran = getRandom('.png')
exec(`ffmpeg -i ${media} ${ran}`, (err) => {
fs.unlinkSync(media)
if (err) return reply('Failed, when converting sticker to image')
buffer = fs.readFileSync(ran)
PhantomRaziOP.sendMessage(from, buffer, image, {quoted: mek, caption: 'Here'})
fs.unlinkSync(ran)
})
break
case 'tts':
        ...
        if (args.length < 2) return PhantomRaziOP.sendMessage(from, 'Where is the text bro??', text, {quoted: mek})
        ngab = budy.slice(7)
        ranm = getRandom('.mp3')
        rano = getRandom('.ogg')
        ngab.length > 600
        ...
        : gtts.save(ranm, ngab, function() {
            exec(`ffmpeg -i ${ranm} -ar 48000 -vn -c:a libopus ${rano}`, (err) => {
                ...
                PhantomRaziOP.sendMessage(from, buff, audio, {quoted:mek,ptt:true})
                fs.unlinkSync(rano)
            })
        })
...
break
    ...
    if ((isMedia && !mek.message.videoMessage || isQuotedImage || isQuotedVideo ) && args.length == 0) {
            ...
            owgi = await PhantomRaziOP.downloadMediaMessage(boij)
...
//══════════[ LEVELING FEATURES ]══════════//
	...
	case 'level':
					...
					PhantomRaziOP.sendMessage(from, resul, text, { quoted: mek})
					.catch(async (err) => {
					console.error(err)
					await reply(`Error!\n${err}`)
					})
					...
					case 'profile':
					if (!isGroup) return reply(mess.group)
				    let anuprofileoke = await PhantomRaziOP.groupMetadata(from)
                    const thu = await PhantomRaziOP.getStatus(anuprofileoke.participants[0], MessageType.text)
    				PhantomRaziOP.updatePresence(from, Presence.composing)
    				try {
					ppimg = await PhantomRaziOP.getProfilePicture(`${sender.split('@')[0]}@s.whatsapp.net`)
					} catch {
					ppimg = 'https://i0.wp.com/www.gambarunik.id/wp-content/uploads/2019/06/Top-Gambar-Foto-Profil-Kosong-Lucu-Tergokil-.jpg'
					}
					profile = `╭─「 *💖ʏᴏᴜʀ ᴘʀᴏꜰɪʟᴇ💖* 」\n│• 𝗡𝗮𝗺𝗲 : ${pushname}\n│• 𝗡𝘂𝗺𝗯𝗲𝗿 : ${sender.split("@")[0]}\n│• 𝗕𝗶𝗼 : ${bio_user}\n│• 𝗫𝗣 : ${getLevelingXp(sender)}\n│• 𝗟𝗲𝘃𝗲𝗹 : ${getLevelingLevel(sender)}\n│• 𝗥𝗮𝗻𝗸 : ${role}\n│• 𝗣𝗠 : wa.me/${sender.split("@")[0]}\n╰──────────────────`
					buffer = await getBuffer(ppimg)
					PhantomRaziOP.sendMessage(from, buffer, image, {quoted: mek, caption: profile})
					break
...
case 'memegenerator': case 'memegen':{
									...
									try {
									   ...
									   var mediiia = await PhantomRaziOP.downloadMediaMessage(enmediaokekek)
										...
										PhantomRaziOP.sendMessage(from, resu, image, {caption:'.stikerin bang', thumbnail: Buffer.alloc(0), quoted: mek})
 	...
 	case 'stickermeme': case 'memesticker': case 'memestick': case 'stickmeme': case 'stcmeme': case 'smeme':{
						if (args.length < 1) return reply(`Send orders *${prefix + command}* PhantomBot`)
									if (q.includes('|')) return reply(`Reply to an image with a caption, For Example *${prefix + command}* Razi`)
									try {
										if (!isQuotedImage) return reply(`Reply to an image!`)
										reply(mess.wait)
										var teks2 = args.join(' ')
										var enmedialel = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
										var mediia = await PhantomRaziOP.downloadMediaMessage(enmedialel)
...
case 'takesticker':
					case 'take':
						...
						const media2 = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediaoo1, `./sticker/${sender}`)
						const packname = ppp.split('|')[0]
						const author = ppp.split('|')[1]
						exif.create(packname, author, `takestick_${sender}`)
						exec(`webpmux -set exif ./sticker/takestick_${sender}.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
							if (error) return reply(mess.error.api)
							PhantomRaziOP.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: mek})
...
case 'stickerwm':
					case 'swm':
						if (isMedia && !mek.message.videoMessage || isQuotedImage) {
							...
							const media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedia9191, `./sticker/${sender}`)
							...
							const media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediaokekak, `./sticker/${sender}`)
							...
							sticWait(from)
								await ffmpeg(`${media}`)
									...
									.on('end', function () {
										...
										exec(`webpmux -set exif ./sticker/stickwm_${sender}.exif ./sticker/${sender}.webp -o ./sticker/${sender}.webp`, async (error) => {
											if (error) return reply(mess.error.api)
											PhantomRaziOP.sendMessage(from, fs.readFileSync(`./sticker/${sender}.webp`), sticker, {quoted: ftex})
											fs.unlinkSync(media)
											fs.unlinkSync(`./sticker/${sender}.webp`)
											fs.unlinkSync(`./sticker/stickwm_${sender}.exif`)
										})
...
const media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedialoli)
ran = '666.webp'
await ffmpeg(`./${media}`)
.input(media)
.on('start', function (cmd) {
...
.on('end', function () {
console.log('Finish')
PhantomRaziOP.sendMessage(from, fs.readFileSync(ran), sticker, {quoted: mek})
fs.unlinkSync(media)
fs.unlinkSync(ran)
})
.addOutputOptions([`-vcodec`, `libwebp`, `-vf`, `scale='min(320,iw)':min'(320,ih)':force_original_aspect_ratio=decrease,fps=15, pad=320:320:-1:-1:color=white@0.0, split [a][b]; [a] palettegen=reserve_transparent=on:transparency_color=ffffff [p]; [b][p] paletteuse`])
...
const media = await PhantomRaziOP.downloadAndSaveMediaMessage(encmedialoli22)
ran = '999.webp'
reply(mess.wait)
await ffmpeg(`./${media}`)
...
var media = await PhantomRaziOP.downloadAndSaveMediaMessage(enmediahe1)
var njay = await imgbb('520bd6f6209077d1777c2a4f20c509c2', media)
var resu = await getBuffer(`https://api.memegen.link/images/custom/${teks1}/${teks2}.png?background=${njay.display_url}`)
PhantomRaziOP.sendMessage(from, resu, image, {quoted: mek})
fs.unlinkSync(media)
} catch (e) {
return reply(`${e}`)
console.log(e)
      ...
      if (!isGroup) return reply(mess.only.group)
        ...
        break
                  ...
                  sange = body.slice(1)
					...
					PhantomRaziOP.sendMessage(from, 'Question : *'+sange+'*\n\nAnswer: '+ nge+'%', text, { quoted: mek })
					break
                case 'gaycheck':
					// source code by Razi⛔
                  gayy = body.slice(1)
					...
					PhantomRaziOP.sendMessage(from, 'Question : *'+gayy+'*\n\nAnswer : '+ yag+'%', text, { quoted: mek })
					break
                case 'lesbicheck':
                case 'lesbiancheck':
					// source code by Razi⛔
                  lesbii = body.slice(1)
					...
					PhantomRaziOP.sendMessage(from, 'Question : *'+lesbii+'*\n\nAnswer : '+ bi+'%', text, { quoted: mek })
					break
                case 'handsomecheck':
					// source code by Razi⛔
ganteng = body.slice(1)
					...
					PhantomRaziOP.sendMessage(from, 'Question : *'+ganteng+'*\n\nAnswer : '+ teng+'%', text, { quoted: mek })
					break
		case 'beautycheck':
					// source code by Razi⛔
cantik = body.slice(1)
					...
					PhantomRaziOP.sendMessage(from, 'Question : *'+cantik+'*\n\nAnswer : '+ tik+'%', text, { quoted: mek })
					break
		case 'charactercheck':
					// source code by Razi⛔
watak = body.slice(1)
					...
					PhantomRaziOP.sendMessage(from, 'Question : *'+watak+'*\n\nAnswer : '+ tak, text, { quoted: mek })
				        break
...
//══════════[ GAME FEATURES ]══════════//
    ...
    "say i love the bot owner Razi through voice note",
    ...
    "breakup with your best friend for 5hrs without telling him/her that its a dare",
     ...
     "send abusive words in any grup, excepting this grup, and send screenshot proof here"
              ...
              PhantomRaziOP.sendMessage(from, buffer, image, { quoted: mek, caption: '_*DARE*_\n'+ der })
              break
                            break
       case 'truth':
              const truth =[
    ...
    "do you love the bot creator, Razi?ðŸ¦„",
    ...
    "do you play pubg, if you then send ur id number"
              ...
              buffer = await getBuffer(`https://i.ibb.co/305yt26/bf84f20635dedd5dde31e7e5b6983ae9.jpg`)
              PhantomRaziOP.sendMessage(from, buffer, image, { quoted: mek, caption: '_*TRUTH*_\n'+ trut })
              break
...
case 'bc':
             ...
             if (args.length < 1) return reply('Where is the text?')
             anu100 = await PhantomRaziOP.chats.all()
             if (isMedia && !PhantomRaziOP.message.videoMessage || isQuotedImage) {
             const encmediaboomb = isQuotedImage ? JSON.parse(JSON.stringify(PhantomRaziOP).replace('quotedM','m')).message.extendedTextMessage.contextInfo : PhantomRaziOP
             bc100 = await PhantomRaziOP.downloadMediaMessage(encmediaboomb)
             for (let _ of anu100) {
             PhantomRaziOP.sendMessage(_.jid, bc100, image, {quoted: fgi, caption: `*「 Phantom Bot Broadcast 」*\n\n${body.slice(4)}`})
}
             reply('Broadcast success')
             } else {
             for (let _ of anu100) {
             PhantomRaziOP.sendMessage(_.jid, 
			{"contentText": `*「 Phantom BOT BROADCASTING MESSAGE 」*\n\n${body.slice(4)}`,
			"footerText": `${tanggal}`,
			"buttons": [
...
anubc2 = await PhantomRaziOP.chats.all()
if (isMedia && !mek.message.videoMessage || isQuotedImage) {
...
buff = await PhantomRaziOP.downloadMediaMessage(encmedia12345)
for (let _ of anubc2) {
PhantomRaziOP.sendMessage(_.jid, buff, image, { viewOnce:true, caption: `${body.slice(4)}`})
}
...
buff = await PhantomRaziOP.downloadMediaMessage(encmediaki)
for (let _ of anubc2) {
PhantomRaziOP.sendMessage(_.jid, buff, video, { viewOnce:true, caption: `${body.slice(4)}`})
}
reply(`Broadcast success ${body.slice(4)}`)
} else if (isMedia && !mek.message.videoMessage || isQuotedVideo) {
...
buff = await PhantomRaziOP.downloadMediaMessage(encmediadirk)
for (let _ of anubc2) {
PhantomRaziOP.sendMessage(_.jid, buff, video, { mimetype: Mimetype.gif, quoted: finv, contextInfo: { forwardingScore: 508, isForwarded: true}, caption: `${body.slice(4)}` })
}
reply(`Broadcast success ${body.slice(4)}`)
} else {
for (let _ of anubc2) {
...
bcgc = await PhantomRaziOP.downloadMediaMessage(encmediabcgc)
for (let _ of groupMembers) {
PhantomRaziOP.sendMessage(_.jid, bcgc, image, { caption: `*「 Phantom BOT BROADCAST 」*\n*Group* : ${groupName}\n\n${body.slice(6)}` })
}
reply('')
} else {
for (let _ of groupMembers) {
sendMess(_.jid, `*「 Phantom BOT BROADCAST 」*\n*Group* : ${groupName}\n\n${body.slice(6)}`)
}
...
if (!isOwner && !mek.key.fromMe) return  reply(mess.only.owner)
anuclearall = await PhantomRaziOP.chats.all()
PhantomRaziOP.setMaxListeners(25)
for (let _ of anuclearall) {
PhantomRaziOP.deleteChat(_.jid)
}
...
media = await PhantomRaziOP.downloadAndSaveMediaMessage(enmediaheheh, './database/sampah/media_user')
await PhantomRaziOP.updateProfilePicture(botNumber, media)
...
PhantomRaziOP.updateProfileName(anuoke112)
...
if (!Number(oi2)) return reply('The number must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  PhantomRaziOP.sendMessage(from, `${oi1}`, MessageType.text)
	  }
...
if (Number(args[0]) >= 50) return reply('Max 50!')
	  ...
	  PhantomRaziOP.sendMessage(from, teks, MessageType.text)
	  }
	...
	if (Number(args[0]) >= 50) return reply('Max 50!')
	  for (let i = 0; i < args[0]; i++) {
	  PhantomRaziOP.sendMessage(from, anutospem, sticker)
	  }
} else if (isQuotedAudio) {
	encmediat = JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo
	            mediat = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediat)
				anutospem2 = fs.readFileSync(mediat)
	if (!Number(args[0])) return reply('Amount must be a number!')
	if (Number(args[0]) >= 50) return reply('Max 50!')
	  for (let i = 0; i < args[0]; i++) {
	  PhantomRaziOP.sendMessage(from, anu2spem2, audio, {mimetype: 'audio/mp4', duration: 359996400, ptt:true})
	  }
} else if (isQuotedImage) {
	boij = isQuotedImage ? JSON.parse(JSON.stringify(mek).replace('quotedM','m')).message.extendedTextMessage.contextInfo : mek
	delb = await PhantomRaziOP.downloadMediaMessage(boij)
	teks = body.slice(6)
	oi1 = teks.split('|')[0]
oi2 = teks.split('|')[1]
if (Number(oi2) >= 50) return reply('Max 50!')
	if (!Number(oi2)) return reply('Amount must be a number!')
	  for (let i = 0; i < oi2; i++) {
	  PhantomRaziOP.sendMessage(from, delb, MessageType.image, {caption: oi1})
	  }
}
	  break
	case 'spam':
				...
				for (let i = 0; i < argzi[1]; i++){
					PhantomRaziOP.sendMessage(from, argzi[0], MessageType.text)
				}
				break
				      case 'leaveall':
             if (!isOwner) return  reply(mess.only.owner)
             let totalgroup = PhantomRaziOP.chats.array.filter(u => u.jid.endsWith('@g.us')).map(u => u.jid)
             for (let id of totalgroup) {
             sendMess(id, 'byee!', null)
             PhantomRaziOP.groupLeave(id)
}
             break
             case 'addvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					...
					PhantomRaziOP.sendMessage(from, `Success, please check with *${prefix}vnlist*`, MessageType.text, { quoted: mek })
					break
					case 'delvn':
					if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					try {
				...
				case 'listvn':
					...
					PhantomRaziOP.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": vien } })
					break
					case 'addimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					...
					PhantomRaziOP.sendMessage(from, `Success, please check with *${prefix}imglist*`, MessageType.text, { quoted: mek })
					break
				case 'delimage':
				if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					try {
				...
				case 'listimage':
					...
					PhantomRaziOP.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": imagi } })
					break
					case 'addsticker':
				if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					if (!isQuotedSticker) return reply('Reply sticker')
					...
					PhantomRaziOP.sendMessage(from, `Success, please check with *${prefix}liststicker*`, MessageType.text, { quoted: mek })
					break
				case 'delsticker':
				if (!isOwner && !mek.key.fromMe) return reply(mess.owner)
					try {
				...
				case 'liststicker':
					...
					PhantomRaziOP.sendMessage(from, teks.trim(), extendedText, { quoted: mek, contextInfo: { "mentionedJid": setik } })
					break
					break
...
//══════════[ GROUP FEATURES ]══════════//
			...
			if (!isGroup) return reply(mess.only.group)
				...
				if (mek.message.extendedTextMessage != undefined){
					...
					PhantomRaziOP.groupCreate(argz[0], anucgc)
					reply(`Success in creating a group ${argz[0]}`)
                }
				break
case 'getbio':
	  var yy = mek.message.extendedTextMessage.contextInfo.participant
var p = await PhantomRaziOP.getStatus(`${yy}`, MessageType.text)
reply(p.status)
if (p.status == 401) {
reply(mess.error.api)
}
...
case 'getdp':
            ...
            pictt = await PhantomRaziOP.getProfilePicture(mentioned)
            pict = await getBuffer(pictt)
            PhantomRaziOP.sendMessage(from, pict, image, {quoted: mek})
            break
                    case 'getname':
        var ambl = mek.message.extendedTextMessage.contextInfo.participant
const sname = PhantomRaziOP.contacts[ambl] != undefined ? PhantomRaziOP.contacts[ambl].notify = undefined ? PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international') : PhantomRaziOP.contacts[ambl].notify || PhantomRaziOP.contacts[ambl].vname : PhoneNumber('+' + ambl.replace('@s.whatsapp.net', '')).getNumber('international')
reply(sname)
break
case 'leave':
            if (!isGroup) return reply(mess.only.group)
                    if (isGroupAdmins || isOwner) {
                    	PhantomRaziOP.groupLeave(from)
                    } else {
...
case 'getdescgc':
				...
				if (!isGroup) return reply(mess.only.group)
					anugetdescgc = from
			   metadete = await PhantomRaziOP.groupMetadata(anugetdescgc)
				PhantomRaziOP.sendMessage(from, metadete.desc, text, {quoted:mek})
				  break
case 'nsfw':
...
await PhantomRaziOP.sendMessage(from, bMess, MessageType.buttonsMessage, {quoted: mek})
break
					case 'opengc':
					if (!isGroup) return reply(mess.only.group)
                   ...
                   reply(`Successful opening group ${groupName}`)
						PhantomRaziOP.groupSettingChange(from, GroupSettingChange.messageSend, false)
						break
						case 'closegc':
						if (!isGroup) return reply(mess.only.group)
                   ...
                   if (!isBotGroupAdmins) return sticNotAdmin(from)
						...
						PhantomRaziOP.groupSettingChange(from, GroupSettingChange.messageSend, true)
					break
case 'grouplink':
case 'gruplink':
case 'gclink':
...
if (!isGroup) return reply(mess.only.group)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
linkgc = await PhantomRaziOP.groupInviteCode(from)
yeh = `https://chat.whatsapp.com/${linkgc}\n\n*${groupName}* group link`
PhantomRaziOP.sendMessage(from, yeh, text, { quoted: fgi })
break
case 'promote' :
...
PhantomRaziOP.groupMakeAdmin(from, mentioned)
} else {
...
mentions(teks, mentioned, true)
PhantomRaziOP.groupDemoteAdmin(from, mentioned)
} else {
...
PhantomRaziOP.groupDemoteAdmin(from, members_id)
break
case 'promoteall':
...
PhantomRaziOP.groupMakeAdmin(from, members_id)
break
case 'add' :
...
PhantomRaziOP.groupAdd(from, [num])
} catch (e) {
console.log('Error :', e)
...
mentioned = mek.message.extendedTextMessage.contextInfo.mentionedJid;
if (mentioned.length > 1) {
PhantomRaziOP.groupRemove(from, mentioned);
reply(mess.success);
} else if (mentioned.length < 1) {
anukick = mek.message.extendedTextMessage.contextInfo.participant;
PhantomRaziOP.groupRemove(from, [anukick]);
reply(mess.success);
} else {
PhantomRaziOP.groupRemove(from, mentioned);
reply(mess.success);
...
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
PhantomRaziOP.groupUpdateSubject(from, `${body.slice(9)}`)
PhantomRaziOP.sendMessage(from, `\`\`\`Success ✅, Renamed the group to\`\`\` *${body.slice(9)}*`, text, { quoted: mek })
break
case 'setdesc':
...
if (!isGroupAdmins) return reply(mess.only.admin)
if (!isBotGroupAdmins) return reply(mess.only.Badmin)
PhantomRaziOP.groupUpdateDescription(from, `${body.slice(9)}`)
PhantomRaziOP.sendMessage(from, `\`\`\`Success ✅, Changing group description\`\`\` *${groupMetadata.subject}* Became: *${body.slice(9)}*`, text, { quoted: fgi })
break
case 'setgrouppp':
case 'setgruppp':
case 'setpp':
...
let media = await PhantomRaziOP.downloadMediaMessage(encmediasetppgc)
PhantomRaziOP.updateProfilePicture(from, media)
...
if (!isGroup) return reply(mess.only.group)
try {
var pic = await PhantomRaziOP.getProfilePicture(from)
} catch {
var pic = 'https://i.ibb.co/Tq7d7TZ/age-hananta-495-photo.png'
}
let ingfo = `*G R O U P I N F O*\n\n*Name :* ${groupName}\n*Group ID :* ${from}\n*Made :* ${moment(`${groupMetadata.creation}` * 1000).tz('Asia/Kolkata').format('DD/MM/YYYY HH:mm:ss')}\n*Group Owner :* @${groupMetadata.owner.split('@')[0]}\n*Number of Admins :* ${groupAdmins.length}\n*Number of participants :* ${groupMembers.length}\n*Welcome :* ${isWelkom ? 'Aktif' : 'Mati'}\n*AntiLink :* ${isAntiLink ? 'Aktif' : 'Mati'}\n*Desc :* \n\n${groupMetadata.desc}`
PhantomRaziOP.sendMessage(from, await getBuffer(pic), image, {quoted: mek, caption: ingfo, contextInfo: {"mentionedJid": [groupMetadata.owner.replace('@c.us', '@s.whatsapp.net')]}})
break
case 'resetlinkgc':
case 'resetlinkgroup':
case 'resetlinkgrup':
...
PhantomRaziOP.query({json, expect200: true})
...
break
       ...
       case 'here':                
             ...
             let online = [...Object.keys(PhantomRaziOP.chats.get(ido).presences), PhantomRaziOP.user.jid]
             PhantomRaziOP.sendMessage(from, 'List Online:\n' + online.map(v => '- @' + v.replace(/@.+/, '')).join `\n`, text, { quoted: mek, contextInfo: { mentionedJid: online }})
             } catch (e) {
             reply(`${e}`)
}
             break
...
case 'sticktag':
            ...
            file = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
            value = args.join(" ")
            var group = await PhantomRaziOP.groupMetadata(from)
            ...
            PhantomRaziOP.sendMessage(from, ini_buffer, sticker, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            PhantomRaziOP.sendMessage(from, ini_buffer, image, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            PhantomRaziOP.sendMessage(from, ini_buffer, audio, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            PhantomRaziOP.sendMessage(from, ini_buffer, video, options)
            fs.unlinkSync(file)
            ...
            ini_buffer = fs.readFileSync(file)
            PhantomRaziOP.sendMessage(from, ini_buffer, document, options)
            fs.unlinkSync(file)
        }  else if ((isMedia && !mek.message.videoMessage || isQuotedVideo) && args.length == 0) {
            encmediau = isQuotedVideo ? JSON.parse(JSON.stringify(mek).replace('quotedM', 'm')).message.extendedTextMessage.contextInfo : mek
            file = await PhantomRaziOP.downloadAndSaveMediaMessage(encmediau, filename = getRandom())
...
PhantomRaziOP.sendMessage(from, options, text, { quoted: mek })
break
case 'contag':
...
reply(`${err}`)
	...
	PhantomRaziOP.sendMessage(`${owner}@s.whatsapp.net`, `─────「 *ALERT-ERROR* 」─────\n\n\`\`\`${e}\`\`\`\n\n────────────────────`, MessageType.text, {contextInfo: { forwardingScore: 508, isForwarded: true, externalAdReply:{title: "Dream Guy Razi",body:"Dont forget to subscribe Razi",previewType:"PHOTO",thumbnail:fs.readFileSync('./media/Phantompic1.jpg'),sourceUrl:"https://wa.me/919744037374"}}})
        }
	// console.log(e)
	}
}
